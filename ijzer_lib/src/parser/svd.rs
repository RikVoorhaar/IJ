...
impl ParseNodeFunctional for Svd {
    fn next_node_functional_impl(
        ...
        let output_type = IJType::Group(vec![
            IJType::Tensor(None),
            IJType::Tensor(None),
            IJType::Tensor(None),
        ]);
...
mod tests {
    ...
    #[test]
    fn test_svd() -> Result<()> {
        let (node, _) = parse_str_no_context("svd [1,2]")?;
        assert_eq!(node.op, Operation::Svd);
        assert_eq!(
            node.output_type,
            IJType::Group(vec![
                IJType::Tensor(None),
        ...
        let (node, _) = parse_str_no_context("(u,s,vt) = svd [1,2]")?;
        ...
        assert_eq!(rhs.op, Operation::Svd);
        Ok(())
    }
}
